...
server <- function(input, output) {
  ...
  pilt_sample <- eventReactive(input$jargmine, {
    pildid_path <- list.files("~/Dropbox/kinnisvara_kuulutused/data/kuulutuste_pildid", 
                              full.names = TRUE)
  ...
  observeEvent(input$grupp, {
    if (input$grupp != 0) {
      ...
      tunnus_path <- str_c("~/Dropbox/kinnisvara_kuulutused/data/kuulutuste_pildid_tunnused/", 
