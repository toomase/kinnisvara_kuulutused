...
server <- function(input, output) {
    ...
    # uuenda automaatselt kui mingi grupi valik on tehtud
    pilt_sample <- eventReactive(input$grupp, {
        pildid_path <- list.files("~/Dropbox/DataScience/R/kinnisvara_kuulutused/data/kuulutuste_pildid", 
                                  full.names = TRUE)
    ...
    observeEvent(input$grupp, {
        if (input$grupp != 0) {
            ...
            tunnus_path <- str_c("~/Dropbox/DataScience/R/kinnisvara_kuulutused/data/kuulutuste_pildid_tunnused/", 
